---
name: create-epic
description: Create exactly one active epic plan under `docs/epics/active/<epic>/epic.md` from a PRD and a user-provided focus. Use after a PRD is ready and the user has chosen one specific end-to-end delivery outcome to plan.
---
...
1. Confirm the epic focus:
   - Use the user's requested PRD area, outcome, workflow, or capability as the epic focus.
   - If the focus is missing or too broad to produce one coherent epic, stop and ask for the specific PRD part to plan.
...
3. Define one epic:
   ...
   - Identify which PRD goals, user stories, constraints, non-goals, and acceptance criteria this epic owns, partially supports, or explicitly does not cover.
...
4. Write the epic:
   ...
   - Include implementation strategy, concern impact, rollout/backout, ADR candidates, PRD coverage role, and cross-epic relationships.
...
5. Report result:
   ...
   - Summarize the focus, PRD coverage role, cross-epic dependencies, assumptions, open questions, and recommend `validate-epic`.
...
- [ ] PRD coverage ownership, partial coverage, and non-coverage are explicit.
