...
description: Validate implemented code against one epic-scoped or standalone story or bugfix change plan and repository quality gates, then update the selected story or bugfix with results. Use after `implement-change` when implementation is complete or partially complete and objective validation is needed before state evolution or archiving.
---
...
1. Load verification context:
   ...
   - Inspect package scripts, CI config, test files, benchmark scripts, migrations, and runbooks referenced by the selected story or bugfix.
...
- [ ] Results map back to placement scope, selected story or bugfix criteria, and implementation task verification.
